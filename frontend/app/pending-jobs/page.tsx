  ...
  const pendingJobs = [
    {
      ...
      description: "Analyzing business data and generating insights.",
    },
  ];

  return (
    <div className="min-h-screen bg-[#f9fafb] dark:bg-zinc-950 text-slate-900 dark:text-zinc-300 p-6 font-sans">
        ...
        <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 dark:border-zinc-800 pb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-zinc-400 italic max-w-md leading-relaxed">
              Jobs waiting for model validation or manual approval before submission.
            </p>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 border-y border-slate-50 dark:border-zinc-800/50 py-3">
              <MetadataItem icon={<MapPin className="w-4 h-4 opacity-70" />} text={`${job.cities[0]}, ${job.countries[0]}`} />
              <MetadataItem icon={<></>} text={`₹${(job.salary_offered / 100000).toFixed(1)} LPA`} />
              <MetadataItem icon={<Globe className="w-4 h-4 opacity-70" />} text={job.is_remote ? "Remote" : "Office"} />
              <MetadataItem icon={<Calendar className="w-4 h-4 opacity-70" />} text={job.start_date} />
            </div>
            ...
            <div className="mt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex flex-wrap gap-2">
                {job.required_skills.map((skill: string) => (
                  <span key={skill} className="text-[12px] font-medium px-2 py-1 bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 text-slate-500 dark:text-zinc-400 rounded hover:bg-amber-50 dark:hover:bg-amber-900/30 transition">
                    {skill}
                  </span>
                ))}
              </div>
